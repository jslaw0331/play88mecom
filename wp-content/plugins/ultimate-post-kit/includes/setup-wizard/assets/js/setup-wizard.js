(function ($) {
    ...
    $(document).ready(function () {
        const wizard = {
            ...
            installButton: document.getElementById('upk-install-plugins-btn'),
            ...
            pluginSliderCheckbox: function(selector){
                ...
                if(pluginSlugs.length){
                    $("#upk-install-plugins-btn").removeClass('d-none').addClass('pulse-animation');
                }else{
                    $("#upk-install-plugins-btn").addClass('d-none').removeClass('pulse-animation');
                }
            },

            onChangedPluginSliderCheckbox: function(){
                const vm = this;
                $('#upk-install-plugins').on('change', '.plugin-slider-checkbox', function (e) {
                    vm.pluginSliderCheckbox('#upk-install-plugins .plugin-slider-checkbox');
            ...
            showStep: function (step) {
                ...
                this.pluginSliderCheckbox('#upk-install-plugins .plugin-slider-checkbox');
            },
            ...
            saveSettingsSubmit: function () {
                const vm = this;
                $('#upk_setup_wizard_modules').submit(function (e) {
                    e.preventDefault();
            ...
            installPlugins: function () {
                const vm = this;
                $('#upk-install-plugins').submit(function (e) {
                    e.preventDefault();
    ...
    $('body').on('click', '.upk-setup-wizard .template-import', function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        const isZipTemplate = $templateCard.hasClass('bdt-upk-import-temp-zip');
        const isJsonTemplate = $templateCard.hasClass('bdt-upk-import-temp-json');
        ...
        if (isJsonTemplate) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'import_upk_elementor_bundle_template',
                    nonce: BDT_SetupWizard.nonce,
                    import_url: importUrl
                },
            ...
            const response = await new Promise((resolve, reject) => {
                $.ajax({
                    url: BDT_SetupWizard.ajax_url, type: 'POST', data: {
                        action: 'import_upk_elementor_bundle_runner_template',
                        nonce: BDT_SetupWizard.nonce,
                        sessionId: sessionId,
                        runner: runner,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    style.textContent = `
        ...
        .item-highlight {
            ...
            box-shadow: 0 0 0 2px var(--upk-primary);
        ...
        .plugin-installed {
            border-color: var(--upk-success) !important;
        }
        ...
        .template-import-failed {
            border-color: var(--upk-danger) !important;
        }
